...
export function ArchiveBrowser({ meta, onClose }: { meta?: Meta; onClose: () => void }) {
  ...
  async function resume(item: ArchiveSummary) {
    ...
    setError('');
    try {
      // Archives always expose a room entry; RoomPage handles seat recovery
      // when the local browser does not yet have a token.
      navigate(`/room/${item.room_id || item.archive_id}`);
    } catch (cause) {
  ...
  return (
    <DialogBackdrop className="archive-backdrop">
      ...
      >
        <button type="button" className="dialog-close" onClick={onClose} aria-label="关闭历史归档">
          <X size={17} />
        </button>
        ...
        <h2 id="archive-browser-title">继续一段旅程</h2>
        <p className="archive-intro">选择一条记录，回到上次离开的回合。网络房间会先进入房间恢复席位。</p>
        {error && (
        ...
        {isLoading && (
          <div className="archive-state">
            ...
            <span>正在读取历史记录…</span>
            <div className="archive-load-progress" role="progressbar" aria-label="正在读取历史归档">
              <span />
            </div>
          </div>
        )}
        {isError && (
          <div className="archive-state">
            <span>历史记录暂时无法读取。</span>
            <button type="button" className="ghost-button" onClick={() => void refetch()}>
              重新读取
            </button>
          </div>
        )}
        {!isLoading && !isError && !archives.length && (
          <div className="archive-state">
            <Archive />
            <span>还没有可继续的旅程。</span>
          </div>
        ...
        <div className="archive-list">
          {archives.map((item) => (
            <article
              className={`archive-card ${loadingId === item.archive_id ? 'is-loading' : ''}`}
              aria-busy={loadingId === item.archive_id}
              key={item.archive_id}
            >
              <div className="archive-card-top">
                <span className="archive-mode">{modeLabels[item.mode] || '历史旅程'}</span>
                <span className="archive-status">{statusLabels[item.status] || '已保存'}</span>
              </div>
              <h3>{scenarios[item.scenario_id] || '云冈遗产旅程'}</h3>
              ...
              <div className="archive-players">
                {item.players.map((player) => (
                  <span key={`${item.archive_id}-${player.name}`}>
                    <b>{player.name}</b>
                    <small>{roles[player.role_id || ''] || '同行角色'}</small>
                  </span>
                ))}
              </div>
              <div className="archive-progress">
                <span>
                  第 {item.turn} / {item.max_rounds} 回合
                </span>
                <Button context="archive" disabled={loadingId !== null} onClick={() => void resume(item)}>
                  {loadingId === item.archive_id ? (
                    <>
                      <LoaderCircle className="spin" />
                      正在加载
                    </>
              ...
              {loadingId === item.archive_id && (
                <div className="archive-card-progress" aria-hidden="true">
                  <span />
                </div>
              )}
            </article>
