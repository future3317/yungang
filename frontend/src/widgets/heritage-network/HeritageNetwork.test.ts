...
describe('map label layout', () => {
  it('does not place visible labels on top of each other or their nodes', () => {
    ...
    const boxes = sites.map((item) =>
      labelBox({ x: item.x!, y: item.y! }, layouts[item.id], Math.min(25, Math.max(8, [...item.name!].length * 1.75)))
    );
    for (let index = 0; index < boxes.length; index += 1) {
      for (let next = index + 1; next < boxes.length; next += 1) {
        expect(
          boxes[index].left >= boxes[next].right ||
            boxes[next].left >= boxes[index].right ||
            boxes[index].top >= boxes[next].bottom ||
            boxes[next].top >= boxes[index].bottom
        ).toBe(true);
      }
    }
  });
  ...
  it('separates coincident node coordinates before rendering the map', () => {
    const positions = computeNodePositions(
      [site('a', '华严寺', 48, 48), site('b', '云冈石窟', 48, 48), site('c', '善化寺', 48, 48)],
      { a: site('a', '华严寺', 48, 48), b: site('b', '云冈石窟', 48, 48), c: site('c', '善化寺', 48, 48) }
    );
