...
function nodeReason(site: Site, meta: Site, current: boolean, target: boolean, eventTarget: boolean) {
  ...
  return `${meta.name || site.id}目前${siteStatusName(site.status)}。点击可查看任务、项目和事件说明。`;
}
...
function labelCandidates(preferred?: string): LabelLayout[] {
  ...
  const directions: Array<[number, number, LabelLayout['anchor']]> = [[1, 0, 'start'], [-1, 0, 'end'], [0, -1, 'middle'], [0, 1, 'middle'], [1, -1, 'start'], [-1, -1, 'end'], [1, 1, 'start'], [-1, 1, 'end']];
  const radii = [5.2, 8.8, 12.8, 17.2, 22.4, 28.4];
  const orderedDirections = preferred === 'left' ? [directions[1], ...directions.filter((_, index) => index !== 1)] : preferred === 'right' ? [directions[0], ...directions.filter((_, index) => index !== 0)] : preferred === 'above' ? [directions[2], ...directions.filter((_, index) => index !== 2)] : preferred === 'below' ? [directions[3], ...directions.filter((_, index) => index !== 3)] : directions;
...
export function computeLabelLayouts(sites: Site[], metas: Record<string, Site>, lod: string, focusedId: string | null, nodePositions: Record<string, Point> = {}) {
  ...
  const ordered = [...visible].sort((a, b) => Number(b.id === focusedId) - Number(a.id === focusedId) || a.id.localeCompare(b.id));
  ordered.forEach(site => {
    ...
    const chosen = candidates.find(candidate => { const box = labelBox(origin, candidate, width); return box.left > 1 && box.right < 99 && box.top > 2 && box.bottom < 98 && !occupied.some(other => intersects(box, other)); });
    if (chosen) { output[site.id] = chosen; occupied.push(labelBox(origin, chosen, width)); }
  });
  return output;
}
...
export function HeritageNetwork({ sites, metaSites, regions = [], routes = {}, players, active, focusedId, reachableIds, actionMode, eventTargetIds = [], eventTargetLabels = [], eventName, onFocus }: { sites: Record<string, Site>; metaSites: Record<string, Site>; regions?: Region[]; routes?: Record<string, RouteState>; players: Player[]; active: Player; focusedId: string | null; reachableIds: ReadonlySet<string>; actionMode: MapActionMode; eventTargetIds?: ReadonlyArray<string>; eventTargetLabels?: string[]; eventName?: string; onFocus: (id: string) => void }) {
  ...
  const regionShapes = useMemo(() => regions.map(region => { const points = region.hull_points?.length ? region.hull_points : region.site_ids.map(id => nodePositions[id] || point(metaSites[id])); return { ...region, shape: softHull(points), label: regionLabelPosition(region, metaSites) }; }).filter(region => region.shape), [metaSites, nodePositions, regions]);
  ...
  useEffect(() => { if (!svgRef.current) return; const selection = select(svgRef.current); const behavior = zoom<SVGSVGElement, unknown>().scaleExtent([.72, 2.2]).on('zoom', event => { transformRef.current = event.transform; setTransform(event.transform); }); zoomRef.current = behavior; selection.call(behavior); requestAnimationFrame(fitBounds); return () => { selection.on('.zoom', null); }; }, [enabledSiteKey]);
  const scaleBy = (factor: number) => { if (svgRef.current && zoomRef.current) select(svgRef.current).call(zoomRef.current.scaleBy, factor); };
  const centerCurrent = () => { const target = mapPoint(active.location); const scale = Math.max(transform.k, 1.12); applyTransform(zoomIdentity.translate(50 - target.x * scale, 50 - target.y * scale).scale(scale), 360); };
  const lod = transform.k < .94 ? 'overview' : transform.k > 1.34 ? 'detail' : 'standard';
  const labelLayouts = useMemo(() => computeLabelLayouts(enabledSites, metaSites, lod, focusedId, nodePositions), [enabledSites, focusedId, lod, metaSites, nodePositions]);
  ...
  return <div className="network-frame world-stage">
    ...
    <svg ref={svgRef} viewBox="0 0 100 100" role="application" aria-label="本局地图，可用 Tab 选择路线和地点" onDoubleClick={fitBounds}>
      <g className="network-atmosphere-layer" aria-hidden="true"><image className="network-atmosphere network-atmosphere-muted" href={assetUrl('interaction/network/network-muted.webp')} x="8" y="14" width="84" height="62" preserveAspectRatio="xMidYMid meet" /><image className="network-atmosphere network-atmosphere-lit" href={assetUrl('interaction/network/network-lit.webp')} x="8" y="14" width="84" height="62" preserveAspectRatio="xMidYMid meet" /></g>
      <g className="map-world" transform={transform.toString()}>
        ...
        <g className="map-focus-layer" aria-hidden="true"><image className="map-player-focus-art" href={assetUrl('interaction/rings/focus.webp')} x={currentPoint.x - 9} y={currentPoint.y - 9} width="18" height="18" preserveAspectRatio="xMidYMid meet" />{selectedPoint && <image className="map-selection-focus-art" href={assetUrl('interaction/rings/neutral.webp')} x={selectedPoint.x - 6.5} y={selectedPoint.y - 6.5} width="13" height="13" preserveAspectRatio="xMidYMid meet" />}</g>
        <g className="route-layer">{visibleLines.map(line => { const target = targetRouteIds.has(line.id); const eventTarget = eventTargetRouteIds.has(line.id); const abnormal = line.route.status !== 'open' || line.route.risk > 0; const classes = [line.route.road_class ? `road-${line.route.road_class}` : '', neighborRouteIds.has(line.id) ? 'is-neighbor' : '', target ? 'is-target' : '', eventTarget ? 'is-event-target' : '', abnormal ? `is-${line.route.status}` : '', line.route.connection_level >= 2 ? 'is-illuminated' : '', hoveredRouteId === line.id || selectedRouteId === line.id ? 'is-hovered' : '', actionMode && !target && !neighborRouteIds.has(line.id) ? 'is-muted' : ''].filter(Boolean).join(' '); return <path key={line.id} className={classes} d={routePath({ ...metaSites[line.from], layout: mapPoint(line.from) }, { ...metaSites[line.to], layout: mapPoint(line.to) }, line.route)} tabIndex={0} role="button" aria-label={`${line.route.name || '路线'}，${line.route.cost} AP，风险 ${line.route.risk}`} onMouseEnter={() => setHoveredRouteId(line.id)} onMouseLeave={() => setHoveredRouteId(null)} onFocus={() => setHoveredRouteId(line.id)} onBlur={() => setHoveredRouteId(null)} onClick={event => { event.stopPropagation(); setSelectedRouteId(line.id); }} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setSelectedRouteId(line.id); } }} />; })}</g>
        <g className="node-layer">{enabledSites.filter(site => lod !== 'overview' || site.node_kind !== 'event').map(site => { const meta = metaSites[site.id] || site; const current = active.location === site.id; const reachable = reachableIds.has(site.id); const target = actionMode !== null && reachable && !current; const eventTarget = eventTargets.has(site.id); const reason = nodeReason(site, meta, current, target, eventTarget); const hoverEndpoint = hoveredRoute && (hoveredRoute.from === site.id || hoveredRoute.to === site.id); const kind = meta.node_kind || 'core'; const size = kind === 'core' ? 4.2 : kind === 'support' ? 3.1 : 2.7; const labelPosition = labelLayouts[site.id]; const icon = meta.icon_asset ? assetUrl(meta.icon_asset) : assetUrl(`generated/nodes/states/${site.id}_${current ? 'active' : site.status === 'closed' ? 'closed' : target ? 'reachable' : 'normal'}.webp`); const frame = site.status === 'closed' || site.status === 'at_risk' ? 'damaged' : kind === 'event' ? 'warning' : kind === 'support' ? 'network' : current ? 'lotus' : 'neutral'; const surface = site.status === 'closed' || site.status === 'at_risk' ? 'damaged' : current ? 'focused' : 'neutral'; const alert = site.status === 'at_risk' || site.status === 'closed'; const position = nodePositions[site.id] || point(meta); return <g key={site.id} className={`map-node node-${kind} ${current ? 'node-current' : ''} ${focusedId === site.id ? 'node-focused' : ''} ${target ? 'node-reachable' : ''} ${alert ? 'node-alert' : ''} ${site.status === 'closed' ? 'node-closed' : ''} ${site.status === 'at_risk' ? 'node-risk' : ''} ${hoverEndpoint ? 'node-route-hover' : ''}`} transform={`translate(${position.x} ${position.y})`} role="button" tabIndex={0} aria-label={`${meta.name || site.id}：${reason}`} onPointerEnter={() => setHoveredSiteId(site.id)} onPointerLeave={() => setHoveredSiteId(null)} onFocus={() => setHoveredSiteId(site.id)} onBlur={() => setHoveredSiteId(null)} onPointerDown={event => { pointerRef.current = { x: event.clientX, y: event.clientY, time: performance.now() }; }} onPointerUp={event => { const start = pointerRef.current; pointerRef.current = null; if (start && Math.hypot(event.clientX - start.x, event.clientY - start.y) < 8 && performance.now() - start.time < 700) onFocus(site.id); }} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onFocus(site.id); } }}><title>{reason}</title><circle className="node-back" r={size} /><image className="node-surface" href={assetUrl(`interaction/surfaces/${surface}.webp`)} x={-size * .92} y={-size * .92} width={size * 1.84} height={size * 1.84} preserveAspectRatio="xMidYMid meet" /><image className="node-frame" href={assetUrl(`interaction/node-frames/${frame}.webp`)} x={-size * .86} y={-size * .86} width={size * 1.72} height={size * 1.72} preserveAspectRatio="xMidYMid meet" /><image className="node-image" href={icon} x={-size * .65} y={-size * .65} width={size * 1.3} height={size * 1.3} preserveAspectRatio="xMidYMid meet" />{alert && <circle className="node-mark" cx={size * .76} cy={-size * .76} r=".8" />}{labelPosition && (kind === 'core' || lod === 'detail' || focusedId === site.id) && <text className="node-label" x={labelPosition.x} y={labelPosition.y} textAnchor={labelPosition.anchor}>{meta.name || site.id}</text>}{labelPosition && lod === 'detail' && (current || target || focusedId === site.id) && <text className="node-status" x={labelPosition.x} y={labelPosition.y + 2.1} textAnchor={labelPosition.anchor}>{current ? '当前位置' : target ? '可选目标' : site.status === 'closed' ? '已关闭' : '查看节点'}</text>}</g>; })}</g>
        {hintSite && (() => { const meta = metaSites[hintSite.id] || hintSite; const current = active.location === hintSite.id; const target = actionMode !== null && reachableIds.has(hintSite.id) && !current; const reason = nodeReason(hintSite, meta, current, target, eventTargets.has(hintSite.id)); const location = mapPoint(hintSite.id); const lines = [reason.slice(0, 25), reason.slice(25, 50), reason.slice(50, 75)].filter(Boolean); const width = Math.min(58, Math.max(34, Math.max((meta.name || hintSite.id).length * 1.8, ...lines.map(line => line.length * 1.15)) + 3)); const height = 5 + lines.length * 2.7; return <g className="node-hover-card" transform={`translate(${Math.max(8, Math.min(100 - width - 8, location.x - width / 2))} ${Math.max(8, location.y - height - 4)})`} pointerEvents="none"><rect width={width} height={height} rx="1.2" /><text x="1.2" y="2.8">{meta.name || hintSite.id}</text>{lines.map((line, index) => <text key={index} x="1.2" y={5.6 + index * 2.5}>{line}</text>)}</g>; })()}
        <g className="event-target-layer" aria-hidden="true">{eventTargetIds.map(id => { const target = metaSites[id]; if (!target) return null; const location = mapPoint(id); return <g key={id} transform={`translate(${location.x} ${location.y})`}><circle className="event-target-ring" r="5.2" /><circle className="event-target-dot" r=".8" /></g>; })}</g>
        <g className="player-marker-layer">{players.map((player, index) => { const location = mapPoint(player.location); const companions = players.filter(item => item.location === player.location); const slot = companions.findIndex(item => item.id === player.id); const angle = companions.length > 1 ? (slot / companions.length) * Math.PI * 2 : -Math.PI / 2; const offset = companions.length > 1 ? 4.3 : 4.8; return <g key={player.id} className={`player-marker marker-${index % 4} ${player.id === active.id ? 'marker-active' : 'marker-idle'}`} transform={`translate(${location.x + Math.cos(angle) * offset} ${location.y + Math.sin(angle) * offset})`} aria-label={`${player.name}，位于${metaSites[player.location]?.name || player.location}`}><circle className="player-marker-halo" r="2.1" /><circle className="player-marker-core" r="1.15" /><text x="0" y=".38">{index + 1}</text></g>; })}</g>
      </g>
    </svg>
    ...
    {eventTargetLabels.length > 0 ? <div className="event-map-notice" role="status"><b>{eventName || '当前事件'}</b><span>影响地点：{eventTargetLabels.join('、')}</span><small>橙色环表示本回合会受影响；橙色实点表示仍可守护。</small></div> : null}
    <div className="network-corner">本局地图<span>{hoveredRoute ? `线路：${routeStatusName(hoveredRoute.route.status)} · ${hoveredRoute.route.cost} AP · 风险 ${hoveredRoute.route.risk}` : actionMode ? `正在选择目标 · 已突出合法线路 · Escape 取消` : '滚轮缩放 · 拖动地图 · 双击适应全部节点'}</span></div>
  </div>;
}
