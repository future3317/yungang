  ...
  return <div className="network-frame world-stage">
    ...
    <svg ref={svgRef} viewBox="0 0 100 100" role="application" aria-label="本局地图，可用 Tab 选择路线和地点" onDoubleClick={fitBounds}>
      ...
      <g className="map-world" transform={transform.toString()}>
        ...
        {hintSite && (() => { const meta = metaSites[hintSite.id] || hintSite; const current = active.location === hintSite.id; const target = actionMode !== null && reachableIds.has(hintSite.id) && !current; const reason = nodeReason(hintSite, meta, current, target, eventTargets.has(hintSite.id)); const location = mapPoint(hintSite.id); return <g className="node-hover-card" transform={`translate(${Math.max(10, Math.min(68, location.x - 16))} ${Math.max(8, location.y - 14)})`} pointerEvents="none"><rect width="32" height="10" rx="1.2" /><text x="1.2" y="2.8">{meta.name || hintSite.id}</text><text x="1.2" y="6.3">{reason.slice(0, 42)}{reason.length > 42 ? '…' : ''}</text></g>; })()}
