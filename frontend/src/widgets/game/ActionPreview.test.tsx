...
describe('ActionPreview', () => {
  it('explains target and cost before confirmation', () => {
    ...
    render(
      <ActionPreview
        action={{ type: 'move', label: '移动到云冈石窟', target_id: 'yungang', cost: 1 }}
        sites={{ yungang: { id: 'yungang', name: '云冈石窟' } as never }}
        cards={{}}
        onConfirm={onConfirm}
        onCancel={vi.fn()}
      />
    ...
    expect(screen.getByText('1 行动点')).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: '确认行动：执行行动' }));
  ...
  it('uses the route name instead of exposing an internal route id', () => {
    render(
      <ActionPreview
        action={{ type: 'use_action_card', label: '使用策略牌', target_id: 'route_shanhua_temple_yungang', cost: 1 }}
        sites={{}}
        routes={{
  ...
  it('shows the backend-provided state changes before confirmation', () => {
    render(
      <ActionPreview
        action={{
          type: 'restore_route',
          label: '修护路线',
          target_id: 'route_shanhua_temple_yungang',
          cost: 1,
          preview_delta: { research_clues: -1, risk: -1 },
        }}
        sites={{}}
        routes={{
          route_shanhua_temple_yungang: { id: 'route_shanhua_temple_yungang', name: '善化寺—云冈石窟' } as never,
    ...
    expect(screen.getByText('研究点 -1 · 路线风险 -1')).toBeInTheDocument();
  });
});
