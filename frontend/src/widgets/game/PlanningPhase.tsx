...
export function PlanningPhase({ state, sites, routes = {}, projects = {}, actions, canAct = true, onChoose }: { state: GameState; sites: Record<string, Site>; routes?: Record<string, RouteState>; projects?: Record<string, ProjectState>; actions: Action[]; canAct?: boolean; onChoose: (action: Action) => void }) {
  ...
  return <Panel className="planning-phase" aria-labelledby="planning-title">
    ...
    <p>每位同行者每轮只能声明一个目标。由另一位同行者完成同一目标，才会触发协作奖励；未被接续不会改变地点、路线或项目状态。</p>
    <div className="planning-marks" aria-live="polite">{marks.length ? marks.map(mark => { const name = resolveTargetName(mark.target_id, sites, routes, projects); const kind = routes[mark.target_id] ? '路线' : projects[mark.target_id] ? '项目' : '地点'; const collaborated = (mark as typeof mark & { collaborated?: boolean | string }).collaborated === true || (mark as typeof mark & { collaborated?: boolean | string }).collaborated === 'true'; const purpose = collaborated ? `已接续：行动点返还 1，研究线索 +1${routes[mark.target_id] ? '，路线风险 -1' : ''}` : '等待另一位同行者接续；未接续不会改变状态'; return <article key={mark.playerId + '-' + mark.target_id}><b>{state.players[mark.playerId]?.name || '队友'}</b><span>{kind} · {name}</span><small>{purpose}</small></article>; }) : <em>还没有人声明本轮目标</em>}</div>
    {!hasCurrentDeclaration && <details className="planning-target-picker"><summary><Plus size={14} />声明一个本轮目标</summary><div className="planning-targets">{targets.map(action => <button key={action.target_id} disabled={!canAct} onClick={() => onChoose(action)}><span>{resolveTargetName(action.target_id, sites, routes, projects) === action.target_id ? localizeActionText(action.label) : resolveTargetName(action.target_id, sites, routes, projects)}</span><small>{action.route_id || routes[action.target_id || ''] ? '路线 · 接续后额外降低风险' : action.target_id && projects[action.target_id] ? '项目 · 接续后获得协作奖励' : '地点 · 接续后获得协作奖励'}</small></button>)}</div></details>}
