...
export function ActionPreview({ action, sites, routes, projects = {}, cards, players = {}, isPending = false, onConfirm, onCancel }: { action: Action; sites: Record<string, SiteReference>; routes?: Record<string, RouteState>; projects?: Record<string, ProjectState>; cards: Record<string, ContentCard>; players?: Record<string, Player>; isPending?: boolean; onConfirm: () => void; onCancel: () => void }) {
  ...
  const intervention = action.type === 'choose_intervention' ? (({ act_now: '立即处理', minimal: '最小干预', record: '先记录' } as Record<string, string>)[action.target_id || ''] || '') : '';
  const heading = action.type === 'move' && targetName !== '当前地点' ? `移动到${targetName}` : localizeActionText(action.label, { sites, routes, projects, players }) || actionLabels[action.type] || '确认行动';
  return <div className="dialog-backdrop"><section className="dialog action-preview" role="dialog" aria-modal="true" aria-labelledby="action-preview-title">
    ...
    <div className="preview-grid"><span><small>目标地点</small><b>{targetName}</b></span>{relation && <span><small>证据关系</small><b>{relation}</b></span>}{intervention && <span><small>处理方式</small><b>{intervention}</b></span>}<span><small>消耗</small><b>{action.cost || 0} AP</b></span>{cardName && <span><small>证据</small><b>{cardName}</b></span>}</div>
