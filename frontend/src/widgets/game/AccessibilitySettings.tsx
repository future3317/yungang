...
function loadPosition(): Position {
  try {
    return { x: 0, y: 0, ...JSON.parse(localStorage.getItem(positionKey) || '{}') };
  } catch {
    return { x: 0, y: 0 };
  }
}
...
export function AccessibilitySettings() {
  ...
  useEffect(() => {
    const move = (event: PointerEvent) => {
      ...
      setPosition({
        x: Math.max(-window.innerWidth + 62, Math.min(0, x)),
        y: Math.max(-window.innerHeight + 62, Math.min(0, y)),
      });
    };
    const end = () => {
  ...
  const style = {
    '--accessibility-drag-x': `${position.x}px`,
    '--accessibility-drag-y': `${position.y}px`,
  } as CSSProperties;
  return (
    <div className="accessibility-control" style={style}>
      <button
        className="accessibility-trigger"
        onPointerDown={handlePointerDown}
        onClick={handleClick}
        title="拖动调整位置，点击打开设置"
        aria-label="打开显示与辅助设置"
      >
        <Settings2 size={18} />
      </button>
      {open && (
        <section className="accessibility-panel" role="dialog" aria-modal="false" aria-label="显示与辅助设置">
          <button className="dialog-close" onClick={() => setOpen(false)} aria-label="关闭设置">
            <X size={16} />
          </button>
          <span className="eyebrow">显示与辅助</span>
          <h2>按你的方式阅读地图</h2>
          <p>拖动圆形按钮可调整位置；偏好仅保存在当前设备。</p>
          <Toggle
...
function Toggle({
  ...
  return (
    <button className="accessibility-toggle" aria-pressed={active} onClick={onClick}>
      <span>
        <b>{label}</b>
        <small>{hint}</small>
      </span>
