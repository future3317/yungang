    ...
    <div className="inspector-content">
      {tab === 'task' && <section id={panelId('task')} className="task-tab" role="tabpanel" aria-labelledby={tabId('task')}>
        {task ? <>
          ...
          {task.required_origin_diversity > 1 && <div className="task-rule-callout"><Info size={15} /><span>这段故事需要 {task.required_origin_diversity} 种线索脉络彼此印证。{task.combo_requirement?.preferred_origins?.length ? `指定线索脉络：${formatRequirementValues(meta, 'origins', task.combo_requirement.preferred_origins)}。` : ''}带回不同来处的证据卡，才能让联系站得住脚。</span></div>}
          ...
          {!isCurrentSite && <div className="task-access-hint"><MapPinned size={15} />你可以先读完这里的记载；抵达节点后，才能亲自寻访证据并归类。</div>}
