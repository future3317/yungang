...
export function StrategyCardDialog({ option, disabled = false, onConfirm, onClose }: { option: ActionOption; disabled?: boolean; onConfirm: (option: ActionOption) => void; onClose: () => void }) {
  ...
  const immediateEffects: Record<string, string> = {
    survey_route: `目标路线风险 ${Number(effect.risk_delta ?? -1) > 0 ? '+' : ''}${Number(effect.risk_delta ?? -1)}，研究线索 +${Number(effect.clues || 0)}`,
    restore_route: '目标路线恢复通行；本次不消耗研究线索。',
    ...
    reserve_ap: `为下一次行动保留 ${Number(effect.amount || 1)} 点行动力。`,
    survey_and_mitigate: '目标路线风险 -1，风化压力 -1。',
    restore_and_move: '恢复目标路线，并获得一次不消耗行动力的移动。',
    transfer_resource: effect.resource === 'ap' ? `为目标同行者增加 ${Number(effect.amount || 1)} 点行动力。` : `将 ${Number(effect.amount || 1)} 点团队修护资源转给目标同行者。`,
