...
export function StrategyCardDialog({ option, disabled = false, onConfirm, onClose }: { option: ActionOption; disabled?: boolean; onConfirm: (option: ActionOption) => void; onClose: () => void }) {
  ...
  return <div className="dialog-backdrop"><section className="dialog strategy-dialog" role="dialog" aria-modal="true" aria-labelledby="strategy-card-dialog-title">
    ...
    <dl><div><dt>使用时机</dt><dd>{localizeActionText(String(payload.timing || '当前行动阶段'))}</dd></div><div><dt>目标</dt><dd>{option.targets.length ? `${option.targets.length} 个合法目标` : '当前地点或团队'}</dd></div><div><dt>最适合</dt><dd>{localizeActionText(String(payload.best_use || option.reason || '根据当前风险选择目标。'))}</dd></div><div><dt>限制</dt><dd>{localizeActionText(String(payload.limitations || option.disabled_reason || '请先选择合法目标。'))}</dd></div><div><dt>预计变化</dt><dd>{previewDeltaText(option.preview_delta, '确认后根据目标结算。')}</dd></div></dl>
