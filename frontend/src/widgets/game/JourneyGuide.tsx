...
export function JourneyGuide({ task, cards, active, legal, meta, actionMode, onChoose }: { task?: Task; cards: Record<string, ContentCard>; active: Player; legal: Action[]; meta: Meta; actionMode: ActionType | null; onChoose: (type: ActionType) => void }) {
  ...
  return <section className="journey-guide" aria-label="行动指引"><div className="section-label"><Target size={14} />旅途中的下一步</div>{actionMode ? <p>风向已经显现，金色标记是此刻可以前往的去处。</p> : task ? <><h3>{task.name}</h3><p>这里的故事需要证据关系，而不只是正确配对。抵达节点后，在研究台判断它们互相说明了什么。</p><ul><li><b>{contributions.length} / {task.required_card_count}</b> 件已归位证据</li><li>{requiredDomains.map(domain => domainName(meta, domain)).join('、')}</li><li>需来自 {task.required_origin_diversity} 种来源</li>{task.combo_requirement?.required_combo_tags?.length ? <li>关键互证：{task.combo_requirement.required_combo_tags.map(tag => contentTagName(tag, meta)).join('、')}</li> : null}</ul><div className="guide-actions">{canInterpret ? <button type="button" onClick={() => onChoose('interpret_evidence')}><HandHeart size={15} />在研究台判断证据 {matchingHand ? `(${matchingHand})` : ''}</button> : null}{canExplore ? <button type="button" onClick={() => onChoose('explore')}><Compass size={15} />寻访一件线索</button> : <button type="button" onClick={() => onChoose('move')}><MapPinned size={15} />沿路线前往节点</button>}</div></> : <><h3>追随下一条线索</h3><p>沿着显影的路线前往节点，在公开市场寻访文化线索；当不同来处的见证相遇，新的联系便会出现。</p><div className="guide-actions"><button type="button" onClick={() => onChoose('move')}><MapPinned size={15} />踏上显影路线</button></div></>}</section>;
}
