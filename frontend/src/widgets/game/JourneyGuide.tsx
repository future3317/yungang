...
export function JourneyGuide({ task, cards, active, legal, meta, actionMode, onChoose }: { task?: Task; cards: Record<string, ContentCard>; active: Player; legal: Action[]; meta: Meta; actionMode: ActionType | null; onChoose: (type: ActionType) => void }) {
  ...
  return <section className="journey-guide" aria-label="行动指引"><div className="section-label"><Target size={14} />旅途中的下一步</div>{actionMode ? <p>风向已经显现，金色标记是此刻可以前往的去处。</p> : task ? <><h3>{task.name}</h3><p>这里的故事还缺几件相互印证的线索。抵达节点，只是寻访的开始。</p><ul><li><b>{contributions.length} / {task.required_card_count}</b> 件线索</li><li>{task.required_domains.map(domain => domainName(meta, domain)).join('、')}</li><li>需来自 {task.required_origin_diversity} 种来处</li>{task.combo_requirement?.required_combo_tags?.length ? <li>组合线索：{task.combo_requirement.required_combo_tags.map(readableTag).join('、')}</li> : null}</ul><div className="guide-actions">{canContribute ? <button onClick={() => onChoose('contribute')}><HandHeart size={15} />交付手中线索 {matchingHand ? `(${matchingHand})` : ''}</button> : null}{canExplore ? <button onClick={() => onChoose('explore')}><Compass size={15} />寻访一件线索</button> : <button onClick={() => onChoose('move')}><MapPinned size={15} />沿路线前往节点</button>}</div></> : <><h3>追随下一条线索</h3><p>沿着显影的路线前往节点，在公开市场寻访文化线索；当不同来处的见证相遇，新的联系便会出现。</p><div className="guide-actions"><button onClick={() => onChoose('move')}><MapPinned size={15} />踏上显影路线</button></div></>}</section>;
}
