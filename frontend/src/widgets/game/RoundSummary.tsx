...
const metricLabels: Record<string, string> = { weathering: '风化压力', weathering_track: '风化压力', threat: '风化压力', restoration_resource: '修护资源', research_clues: '研究线索', influence: '共同影响', route_risk: '路线风险', damage: '节点损伤' };
...
export function RoundSummary({ state, sites, onContinue }: { state: GameState; sites: Record<string, Site>; onContinue?: () => void }) {
  ...
  return <section className="round-summary" aria-label="上一回合摘要">
    <div className="round-summary-main"><CheckCircle2 size={18} /><span><b>上一回合已结算</b><small>事件目标：{targetNames || '暂无指定目标'}</small>{resolutions.map((item, index) => <span key={`${item.label || 'result'}-${index}`}><small>{item.label || '事件结果'}{item.reason ? ` · ${item.reason}` : ''}</small><StateChangeList compact changes={Object.entries(item.changes || {}).map(([metric, value]) => ({ label: metricLabels[metric] || '状态变化', delta: typeof value === 'number' ? value : null, after: value }))} /></span>)}</span></div>
