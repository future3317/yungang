...
export function ScenarioHeader({ state, scenarioName, connection, eventSummary }: { state: GameState; scenarioName: string; connection: string; eventSummary?: { name?: string; targets?: string[]; historyCount?: number } }) {
  const [goalsOpen, setGoalsOpen] = useState(false);
  const goalDrag = useDraggablePosition('yungang-goal-hud-position-v1', { minVisibleWidth: 320, minVisibleHeight: 58 });
  ...
  const conditionRow = (item: (typeof victory)[number], failureCondition = false) => {
    ...
    return <span key={item.id} className={complete ? (failureCondition ? 'condition-danger' : 'condition-done') : ''}><i>{complete ? (failureCondition ? '!' : '✓') : '·'}</i><b>{item.label}</b> {item.current} / {item.target}{item.id === 'weathering_control' ? <small>上限以内</small> : item.remaining > 0 && <small>{failureCondition ? `余量 ${item.remaining}` : `还差 ${item.remaining}`}</small>}{related && <small className="condition-related">{related}</small>}</span>;
  };
  return <header className="game-header"><div className="brand-group"><a className="brand-mark" href="/" aria-label="返回首页"><img src={assetUrl('ornaments/yungang-seal-stamp.png')} alt="" /><span>云冈</span></a><span className="brand-scenario" title={`本局主题：${scenarioName}`}><small>本局主题</small><b>{scenarioName}</b></span><div className="event-history-bar top-event-summary" role="status" aria-live="polite"><span className="event-history-kicker">{eventSummary?.name ? '当前事件' : '历史事件'}</span><b>{eventSummary?.name || '旅程事件记录'}</b><span>{eventSummary?.targets?.length ? `影响地点：${eventSummary.targets.join('、')}` : `${eventSummary?.historyCount || 0} 条事件记录`}</span></div></div><div className="header-center"><section className={`goal-hud ${goalsOpen ? 'is-open' : ''}`} data-draggable-surface="true" style={goalDrag.style} aria-label="共同目标进度" aria-expanded={goalsOpen}><div className="goal-hud-head"><button type="button" className="goal-drag-handle" aria-label="拖动共同目标面板" title="拖动调整共同目标位置" onPointerDown={goalDrag.onPointerDown} onClickCapture={goalDrag.onClickCapture}><GripVertical size={15} aria-hidden="true" /></button><span className="goal-title">共同目标</span><span className="goal-phase">第 {state.shared.turn} / {state.shared.max_rounds} 回合 · {phase}</span></div><div className="goal-summary">{visibleVictory.length ? visibleVictory.slice(0, 2).map(item => <span key={item.id}>{item.label} {item.current} / {item.target}</span>) : <><span>核心项目 {completedProjects} / {projectTarget}</span><span>公共目标 {completedObjectives} / {objectiveTarget}</span></>}<span>风化压力 {weathering} / {weatheringLimit}</span><span>剩余 {goal?.rounds_remaining ?? state.shared.max_rounds - state.shared.turn + 1} 回合</span><Progress value={totalProgress} max={totalTarget} /></div><button className="goal-toggle" type="button" aria-expanded={goalsOpen} aria-controls="goal-conditions" onClick={() => setGoalsOpen(open => !open)}>{goalsOpen ? '收起胜利条件' : '查看胜利条件'}<span aria-hidden="true">{goalsOpen ? '⌃' : '⌄'}</span></button>{goalsOpen && <div id="goal-conditions" className="goal-conditions" tabIndex={0}><div><b>要完成</b>{victory.map(item => conditionRow(item))}</div><div><b>不能超过</b>{failure.map(item => conditionRow(item, true))}</div></div>}</section></div><div className="header-actions">{connection !== '已连接' && <span className={`sync-state ${connection === '离线' ? 'offline' : ''}`} aria-live="polite"><span />{connection}</span>}</div></header>;
}
