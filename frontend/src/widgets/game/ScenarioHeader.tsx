...
export function ScenarioHeader({ state, scenarioName, connection, gameReference, eventSummary, onFocusGoal }: { state: GameState; scenarioName: string; connection: string; gameReference?: string; eventSummary?: { name?: string; targets?: string[]; historyCount?: number }; onFocusGoal?: (ids: string[]) => void }) {
  ...
  const conditionRow = (item: (typeof victory)[number], failureCondition = false) => {
    ...
    return <button type="button" key={item.id} className={`condition-row ${failureCondition ? (item.status === 'failed' ? 'condition-danger' : 'condition-safe') : complete ? 'condition-done' : ''}`} disabled={!onFocusGoal || ids.length === 0} onClick={() => onFocusGoal?.(ids)}><i>{failureCondition ? (item.status === 'failed' ? '!' : safe ? '✓' : '·') : complete ? '✓' : '·'}</i><b>{item.label}</b> {item.current} / {item.target}{failureCondition ? <small>{item.status === 'failed' ? '已超限' : `还可承受 ${item.remaining}`}</small> : item.remaining > 0 && <small>还差 {item.remaining}</small>}{related && <small className="condition-related">{related}</small>}</button>;
  };
