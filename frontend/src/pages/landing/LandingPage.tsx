...
export function LandingPage() {
  ...
  return <main className="landing"><div className="landing-copy"><div className="eyebrow">云冈 · 数字遗产协作体验</div><h1>石窟<br /><em>光谱</em></h1><p>一张会随着探索、修护与协作逐步显影的文化关系图。让散落在时间中的证据重新彼此照见。</p><div className="landing-facts"><span><Compass />{selectedScenario?.recommended_minutes || '20-35'} 分钟</span><span><Users />1-4 位同行者</span><span><BookOpen />共同修复遗产网络</span></div></div><section className="start-panel" aria-labelledby="start-title"><div className="panel-kicker">点亮一段共同旅程</div><h2 id="start-title">从一束光开始</h2><p>{isError ? '场景资料暂时离席，仍可使用默认旅程。' : '选择同行方式、场景与难度，随后在准备厅确认角色与席位。'}</p>{createError && <div className="landing-feedback" role="alert">{createError}<button type="button" onClick={() => setCreateError('')}>知道了</button></div>}<form onSubmit={create}><label htmlFor="player-name">你的名字<input id="player-name" className="player-name-input" value={name} maxLength={24} onChange={event => setName(event.target.value)} placeholder="例如：林工" /></label><fieldset className="play-mode-picker"><legend>同行方式</legend><div className="play-mode-options">{(['solo', 'local', 'multi_device'] as PlayMode[]).map(item => <button type="button" key={item} className={mode === item ? 'selected' : ''} onClick={() => setMode(item)} aria-pressed={mode === item}><span>{item === 'solo' ? <Compass /> : item === 'local' ? <Laptop /> : <Wifi />}</span><b>{item === 'solo' ? '单人旅程' : item === 'local' ? '本地协作' : '多设备房间'}</b><small>{modeNotes[item]}</small></button>)}</div></fieldset>{mode !== 'solo' && <label>房间席位<div className="segmented">{[2, 3, 4].map(value => <button type="button" key={value} className={count === value ? 'selected' : ''} onClick={() => setCount(value)} aria-pressed={count === value}>{value} 位</button>)}</div></label>}<section className="scenario-picker"><div className="scenario-heading"><span>旅程场景</span><small>不同路线压力与共同目标</small></div><div className="scenario-options">{scenarios.map(item => <button type="button" key={item.id} className={scenario === item.id ? 'selected' : ''} onClick={() => setScenario(item.id)} aria-pressed={scenario === item.id}><b>{item.name}</b><small>{item.description || '一段等待被重新连起的遗产旅程。'}</small>{item.victory_brief && <em>胜利：{item.victory_brief}</em>}<i>{item.recommended_players?.length ? `推荐 ${item.recommended_players.join('、')} 人 · ` : ''}{item.max_rounds || '—'} 回合</i></button>)}</div>{selectedScenario && <div className="scenario-detail"><b>本局核心机制</b><span>{selectedScenario.scenario_rule?.description || '探索、互证与修护共同推进这段旅程。'}</span><small>失败：{selectedScenario.failure_brief || '回合耗尽或风化压力失控。'}</small></div>}</section><label>旅程难度<select value={difficulty} onChange={event => setDifficulty(event.target.value)}>{difficulties.map(item => <option key={item.id} value={item.id}>{item.name || difficultyLabels[item.id] || '未标注难度'}</option>)}</select>{selectedDifficulty && <small className="field-note">{selectedDifficulty.description} · {selectedDifficulty.max_rounds} 回合 · 初始修护资源 {selectedDifficulty.restoration_resource}{selectedDifficulty.solo_ap_bonus ? ` · 单人额外 ${selectedDifficulty.solo_ap_bonus} AP` : ''}</small>}</label><button type="button" className="advanced-toggle" onClick={() => setShowSeed(value => !value)} aria-expanded={showSeed}><Dice5 size={15} />旅程种子：{showSeed ? '收起' : '高级设置'}</button>{showSeed && <label htmlFor="seed">可复现种子<input id="seed" inputMode="numeric" value={seedText} onChange={event => setSeedText(event.target.value)} placeholder="留空则由旅程决定" /></label>}<button className="primary-cta" disabled={busy}>{busy ? '正在点亮路线…' : '进入准备厅'}<ArrowRight /></button></form><div className="resume"><label htmlFor="room-code">回到已有旅程或房间</label><div><input id="room-code" value={roomCode} onChange={event => setRoomCode(event.target.value)} placeholder="输入房间码或旅程编号" /><button type="button" disabled={!roomCode.trim()} onClick={resume}>进入</button></div></div></section></main>;
}
