...
export function GamePage() {
  ...
  const [dismissedTutorialIntent, setDismissedTutorialIntent] = useState<TutorialContext | null>(null);
  ...
  const enqueueToast = (text: string) => {
    const id = crypto.randomUUID();
    setToasts((items) => [...items.slice(-3), { id, text }]);
    window.setTimeout(() => setToasts((items) => items.filter((item) => item.id !== id)), 4800);
  };
  const gameQuery = useQuery<GameState>({
    queryKey: ['room-game', roomId, roomToken],
    queryFn: () => api.roomGame(roomId, roomToken),
    refetchOnWindowFocus: false,
    refetchInterval: fallbackPollInterval(roomEventState, roomId),
  });
  useRoomEvents({
    roomId,
    token: roomToken,
    onRevision: () => {
      void queryClient.invalidateQueries({ queryKey: ['room-game', roomId, roomToken] });
    },
    onState: setRoomEventState,
  });
  ...
  useEffect(() => {
    if (
      tutorialOpen &&
      (preview || card || strategyOption || selectedOption || completedTask || handoffName || state?.pending_choice)
    ) {
  ...
  const legal = actionOptions.flatMap((option) =>
  ...
  const mutation = useMutation({
    mutationFn: (action: Action) => api.roomAction(roomId, roomToken, action, state?.revision || 0),
    onSuccess: (data, action) => {
      queryClient.setQueryData(['room-game', roomId, roomToken], data);
      setPreview(null);
      ...
      if (action.type === 'choose_intervention') {
        const newlyCompleted = Object.values(data.tasks).find(
          (nextTask) => nextTask.completed && !state?.tasks[nextTask.id || '']?.completed
        );
        if (newlyCompleted) {
          const completedSite = Object.values(data.sites).find((item) => item.active_task_id === newlyCompleted.id);
          setCompletedTask({
            task: newlyCompleted,
            siteName: completedSite?.id || '当前节点',
            changes: (data.feedback_events || []).flatMap((event) => event.changes || []),
          });
        }
      }
      if (
        data.viewer?.play_mode === 'local' &&
        action.type === 'end_turn' &&
        data.players[data.shared.active_player_id]
      )
        setHandoffName(data.players[data.shared.active_player_id].name);
      if (
        action.type === 'end_turn' &&
        state?.shared.current_event_id !== data.shared.current_event_id &&
        data.shared.current_event_id
      ) {
        const settledName = currentEvent?.name || '上一轮事件';
        const nextName = meta.events?.find((item) => item.id === data.shared.current_event_id)?.name || '新的世界事件';
        enqueueToast(`${settledName}已结算，${nextName}已揭示。`);
      }
      (data.feedback_events?.length
        ? data.feedback_events
        : [{ message: actionFeedback(action, state, data), changes: [] }]
      ).forEach((event, index) =>
        window.setTimeout(
          () => enqueueToast([event.message, ...feedbackChangeText(event.changes || [])].filter(Boolean).join(' · ')),
          index * 180
        )
      );
    },
    onError: (error) => {
      if (error instanceof ApiError && error.status === 409) {
        ...
        if (current) {
          queryClient.setQueryData(['room-game', roomId, roomToken], current);
          setActionMode(null);
          setSelectedOption(null);
          setPreview(null);
          setCard(null);
          setStrategyOption(null);
          enqueueToast('\u72b6\u6001\u5df2\u540c\u6b65\uff0c\u8bf7\u91cd\u65b0\u9009\u62e9\u884c\u52a8\u3002');
          return;
        }
      }
      setActionMode(null);
      setSelectedOption(null);
      setPreview(null);
      enqueueToast(localizeActionError(error, metaQuery.data));
    },
  });
  useEffect(() => {
  ...
  if (gameQuery.isError || metaQuery.isError || !state || !metaQuery.data)
    return (
  ...
  if (state.shared.outcome)
    return <Navigate to={roomId ? `/room/${roomId}/result` : `/result/${state.session_id}`} replace />;
  ...
  const connection =
    mutation.isPending || gameQuery.isFetching || metaQuery.isFetching
      ? '同步中'
      : roomEventState === 'unauthorized'
        ? '席位失效'
        : roomEventState === 'room_ended'
          ? '旅程已结束'
          : roomEventState === 'retrying'
            ? '重连中'
            : roomEventState === 'ended'
              ? '重新连接'
              : '已连接';
  const run = (action?: Action) => {
    if (roomId && roomEventState === 'unauthorized') {
      enqueueToast('\u5e2d\u4f4d\u51ed\u8bc1\u5df2\u5931\u6548\uff0c\u8bf7\u8fd4\u56de\u623f\u95f4\u6062\u590d\u3002');
      return;
    }
    if (action && canAct && !mutation.isPending)
      mutation.mutate({ ...action, request_id: action.request_id || crypto.randomUUID() });
  };
  const announceIntent = (type: ActionType) => {
    const context = tutorialContextForAction(type);
    if (context && context !== dismissedTutorialIntent && !tutorialProgress.hasSeenContext(context)) {
      setTutorialIntent(context);
      setTutorialOpen(true);
    }
  };
  const chooseOption = (option: ActionOption) => {
    if (!canAct || option.enabled === false) return;
    announceIntent(option.type);
    if (option.type === 'explore' && option.targets.length !== 1) {
      setSelectedOption(null);
    ...
    if (option.targets.length === 1) {
      selectAction(optionAction(option, option.targets[0]));
      return;
    }
    if (option.targets.length) {
      setSelectedOption(option);
      if (['move', 'restore', 'survey_route', 'restore_route', 'establish_connection'].includes(option.type)) {
        setActionMode(option.type as ActionMode);
        setFocus(active.location);
      } else setActionMode(null);
      return;
    }
    const action = optionAction(option);
    if (option.type === 'end_turn') {
      const weathering = state.shared.weathering_track;
      const isFinalPlayer = state.shared.player_order[state.shared.player_order.length - 1] === active.id;
      const warnings = [
        active.ap > 0 ? `你还有 ${active.ap} 点行动点未使用` : '',
        isFinalPlayer && state.shared.current_event_id ? '当前事件将在回合结束时结算' : '',
        weathering >= Math.max(0, (state.shared.weathering_limit || 5) - 1) ? '风化压力已经接近上限' : '',
      ].filter(Boolean);
      const handoff = isFinalPlayer
        ? '确认结束团队本轮行动，随后进入事件结算。'
        : '确认结束这位角色的行动，交给下一位同行者；事件暂不结算。';
      setPreview({ ...action, description: warnings.length ? `${warnings.join('；')}。${handoff}` : handoff });
      return;
    }
    if (['use_skill', 'use_node_ability', 'use_upgrade', 'end_planning', 'prepare'].includes(option.type))
      setPreview(action);
    else run(action);
  };
  const chooseAction = (type: ActionType) => {
    const option = state.action_options?.find((item) => item.type === type && item.enabled !== false);
  ...
  const selectNode = (id: string) => {
    const target =
      mapActionMode && selectedOption?.type === actionMode
        ? selectedOption.targets.find((item) => {
            const payload = item.payload || {};
            return (
              item.id === id || payload.target_id === id || payload.target_site_id === id || payload.route_id === id
            );
          })
        : undefined;
    if (target && selectedOption) {
      selectAction(optionAction(selectedOption, target));
      return;
    }
    setFocus(id);
    setFocusCardOpen(true);
  ...
  const mapActionMode =
    actionMode && ['move', 'restore', 'survey_route', 'restore_route', 'establish_connection'].includes(actionMode)
      ? (actionMode as Extract<
          ActionType,
          'move' | 'restore' | 'survey_route' | 'restore_route' | 'establish_connection'
        >)
      : null;
  const selectExploreCard = (id: string) => {
    const option = state.action_options?.find(
      (item) => item.type === 'explore' && item.targets.some((target) => target.payload?.card_id === id)
    );
    const target = option?.targets.find((item) => item.payload?.card_id === id);
    if (option && target) selectAction(optionAction(option, target));
  };
  const selectInterpretation = (cardId: string, relation: 'support' | 'conflict' | 'pending') => {
    announceIntent('interpret_evidence');
    const action = legal.find(
      (item) => item.type === 'interpret_evidence' && item.card_id === cardId && item.target_id === relation
    );
    if (action) selectAction(action);
  };
  const formInterpretation = () => {
    const action = legal.find((item) => item.type === 'form_interpretation');
    if (action) selectAction(action);
  };
  const chooseIntervention = (choice: 'act_now' | 'minimal' | 'record') => {
    const action = legal.find((item) => item.type === 'choose_intervention' && item.target_id === choice);
    if (action) selectAction(action);
  };
  const pendingAction = (action: Action) =>
    state.pending_choice?.kind === 'action_card'
      ? (announceIntent('use_action_card'), selectAction(action))
      : run(action);

  const journalEntries = state.shared.journal || [];
  const historyEntries = (state.shared.event_history || []).map((entry, index) => ({
    id: `event-history-${entry.event_id || index}-${entry.round || index}`,
    round: Number(entry.round || 0),
    type: 'event',
    message: `${events[entry.event_id || '']?.name || '世界事件'}已结算`,
    effects: (entry.resolution || []).flatMap(
      (item: {
        changes?:
        ...
        Array.isArray(item.changes)
          ? item.changes.map((change) => ({ ...change, label: change.label || metricLabel(change.metric || '') }))
          : Object.entries(item.changes || {}).map(([key, value]) => ({
              label: metricLabel(key),
              delta: typeof value === 'number' ? value : undefined,
              after: typeof value === 'number' ? undefined : String(value),
            }))
    ),
    created_at: '',
    player_id: undefined,
  ...
  const timelineEvents = [
    ...journalEntries.filter((entry) => !String(entry.message).startsWith('事件结算：')),
    ...historyEntries,
  ].map((entry) => ({
    ...entry,
      ...
      <div className="game-shell">
        <HudSlot name="top">
          <ScenarioHeader
            state={state}
            scenarioName={
              meta.scenarios?.find((item) => item.id === (state.scenario_id || state.shared.scenario_id))?.name ||
              '未命名旅程'
            }
        ...
        <main className={`hud-layout ${inspectorOpen ? '' : 'inspector-collapsed'}`}>
          ...
          >
            <section className="roster">
              ...
              {state.shared.player_order.map((id) => {
                const player = state.players[id];
                const role = roles[player.role_id];
                const skillHint = role?.ability
                  ? `${role.ability.name}：${role.ability.description || '这项能力会改变这位角色最擅长的行动。'}`
                  : undefined;
                return (
                  <button
                    key={id}
                    className={`roster-row ${id === state.shared.active_player_id ? 'active' : ''}`}
                    title={skillHint}
                    aria-label={`${player.name}，${role?.name || player.role_id}${skillHint ? `，技能：${skillHint}` : ''}`}
                    onClick={() => setFocus(player.location)}
                  >
                    <img src={assetUrl(roleBadgeAsset(player.role_id, role?.icon_asset || undefined))} alt="" />
                    <span>
                      <b>{player.name}</b>
                      <small>
                        {role?.name || player.role_id} · {sites[player.location]?.name || player.location}
                      </small>
                      {skillHint && (
                        <em className="roster-skill-popover" role="tooltip">
                          <b>技能：{role?.ability?.name}</b>
                          <span>{role?.ability?.description || '这项能力会改变这位角色最擅长的行动。'}</span>
                        </em>
                      )}
                    </span>
                    <strong>
                      {player.ap} <small>行动点</small>
                    </strong>
                  </button>
                );
              })}
            ...
            <section className="current-player">
              <div className="section-label">当前行动者</div>
              <h2>{active.name}</h2>
              <p>{roles[active.role_id]?.name || active.role_id}</p>
              {roles[active.role_id]?.ability && (
                <span className="role-skill-tip" tabIndex={0}>
                  <Sparkles size={13} />
              ...
              <div className="player-resources">
                <span title="本回合可用于移动、寻访证据和其他行动">
                  <b>{active.ap}</b>
                  <small>行动点</small>
                </span>
                <span title="记录这位角色对团队解释的贡献">
                  <b>{active.influence}</b>
                  <small>个人声望</small>
                </span>
                <span title="团队修护资源不足时，可由这位角色代付修护">
                  <b>{active.supplies || 0}</b>
                  <small>个人补给</small>
                </span>
              </div>
            </section>
          ...
          <SiteInspector
            ...
            onCollapsedChange={setInspectorOpen}
            onExplore={selectExploreCard}
            actionOptions={actionOptions}
        ...
        <GameOverlayHost>
          {roomId && roomEventState === 'unauthorized' && (
            <div className="room-state-notice" role="alert">
              <h2>\u5e2d\u4f4d\u9700\u8981\u6062\u590d</h2>
              <p>
                \u5f53\u524d\u5e2d\u4f4d\u51ed\u8bc1\u5df2\u5931\u6548\uff0c\u8bf7\u56de\u5230\u623f\u95f4\u91cd\u65b0\u6062\u590d\u540c\u4e00\u5e2d\u4f4d\u3002
              </p>
              <Button onClick={() => window.location.assign('/room/' + roomId)}>
                \u8fd4\u56de\u623f\u95f4\u6062\u590d
              </Button>
            </div>
          )}
          ...
          {completedTask && (
            <TaskCompleteDialog
              ...
              siteName={sites[completedTask.siteName]?.name || focusedMeta.name || '当前节点'}
              changes={completedTask.changes}
          ...
          {card && (
            <EvidenceCardDialog
              id={card}
              item={cards[card]}
              action={findCardAction(legal, 'play_card', card)}
              interpretActions={legal.filter((item) => item.type === 'interpret_evidence' && item.card_id === card)}
              onClose={() => setCard(null)}
              onUse={(action) => {
                setCard(null);
                if (action) setPreview(action);
              }}
            />
          )}
          {state.pending_choice && (
            <ChoiceDialog state={state} event={currentEvent} disabled={mutation.isPending} onChoose={pendingAction} />
          )}
          {selectedOption && !mapActionMode && (
            <ActionTargetDialog
              ...
              onChoose={(target) => selectAction(optionAction(selectedOption, target))}
              onCancel={() => {
                setActionMode(null);
                setSelectedOption(null);
              }}
          ...
          <TutorialGuide
            ...
            onOpenChange={(open) => {
              ...
              if (!open) {
                if (tutorialIntent) setDismissedTutorialIntent(tutorialIntent);
                setTutorialIntent(null);
              }
            }}
            state={state}
          ...
          {strategyOption && (
            <StrategyCardDialog
              ...
              disabled={mutation.isPending}
              onClose={() => setStrategyOption(null)}
              onConfirm={(option) => {
                setStrategyOption(null);
                chooseOption(option);
              }}
  ...
  return (
    <div className="dialog-backdrop">
      ...
      >
        ...
        <h2 id="task-complete-title">{task.name}</h2>
        <p>{siteName} 的线索已彼此照见。你们已经完成一段可被共同引用的文化解释。</p>
        <div className="task-complete-reward">
          <b>本次改变</b>
          {changes.length ? (
            <StateChangeList changes={changes} />
          ) : (
            <span>解释已写入遗产网络，地图与旅程记录已更新。</span>
          )}
          <span>对应领域印记已加入图卷</span>
        </div>
...
function ActionTargetDialog({
  ...
  disabled: boolean;
  onChoose: (target: ActionOption['targets'][number]) => void;
  onCancel: () => void;
}) {
  const ref = useDialogFocus();
  return (
    <div className="dialog-backdrop">
      <section
        ...
        aria-describedby="action-target-description"
      >
        <button className="dialog-close" disabled={disabled} onClick={onCancel} aria-label="取消目标选择">
          <X />
        </button>
        ...
        {option.requirements?.length ? (
          <div className="action-requirements" aria-label="行动前提">
            <b>行动前提</b>
            <span>{option.requirements.join(' · ')}</span>
          </div>
        ) : null}
        ...
        <button className="ghost-button" disabled={disabled} onClick={onCancel}>
          返回浏览
        </button>
      </section>
    </div>
...
function ChoiceDialog({
  ...
  const pendingCopy: Record<string, { eyebrow: string; title: string; body: string }> = {
    discard: {
      eyebrow: '证据卡整理 · 需要选择',
      title: '证据卡已满',
      body: '新的线索已经抵达。请选择一张旧线索放入弃牌堆，才能把新线索带回研究台。',
    },
    view_select: {
      eyebrow: '档案检视 · 需要选择',
      title: '从档案中挑出一条线索',
      body: '档案库已摊开可检视的线索。选择一条加入证据卡，其他线索会留在档案中。',
    },
    archive_select: {
      eyebrow: '档案检视 · 需要选择',
      title: '确定要记录的档案线索',
      body: '你正在查看档案顶部的线索。选择一条作为本次研究的重点，选择完成后才能继续行动。',
    },
    archive_retrieve: {
      eyebrow: '档案回收 · 需要选择',
      title: '从档案中取回一条线索',
      body: '角色专长允许你调换一张证据卡与档案线索。选择要取回的线索，随后再选择要放回档案的证据卡。',
    },
    action_card: {
      eyebrow: '策略牌 · 选择目标',
      title: '为策略牌选择目标',
      body: '这张策略牌已经准备好。选择一个合法目标，确认后费用和效果会立即结算。',
    },
    select_market_card: {
      eyebrow: '公开市场 · 需要选择',
      title: '选择一条带回的证据卡',
      body: '公开市场已经展开。选中的线索会进入当前角色的证据卡，并推进后续的研究台判断。',
    },
    choice: {
      eyebrow: '共同决定 · 需要选择',
      title: '完成当前选择',
      body: '这一步会影响团队的资源、风险或证据进度。选择一个选项后，旅程才能继续。',
    },
  };
  const copy = isEvent
    ? {
        eyebrow: '世界事件 · 需要回应',
        title: event?.name || '线路正在等待回应',
        body: event?.forecast_text || event?.description || '你的选择会立即改变本回合的风险与资源。',
      }
    : pendingCopy[pendingKind] || pendingCopy.choice;
  const decisionBrief = eventDecisionBrief(event);
  const ref = useDialogFocus();
  if (state.pending_choice?.kind === 'role_upgrade')
    return <RoleUpgradeDialog state={state} disabled={disabled} onChoose={onChoose} />;
  const choices = (state.action_options || []).flatMap((option) =>
    option.targets.length ? option.targets.map((target) => optionAction(option, target)) : [optionAction(option)]
  );
  return (
    <div className="dialog-backdrop">
      ...
      >
        ...
        <p>{copy.body}</p>
        {isEvent && (
          <div className="choice-hint choice-impact">
            <b>不处理会怎样</b>
            <span>{decisionBrief.ifIgnored}</span>
          </div>
        )}
        {isEvent && (
          <div className="choice-hint">
            <b>现在能做什么</b>
            <span>{decisionBrief.whatYouCanDo}</span>
          </div>
        )}
        {isEvent && state.shared.event_targets?.length ? (
          <div className="choice-hint">
            <b>已锁定影响范围</b>
            <span>{state.shared.event_targets.length} 个目标已由本局种子确定，选择会改变这些目标的结果。</span>
          </div>
        ) : null}
        <div className="choice-list">
          {choices.map((action, index) => (
            <button disabled={disabled} key={`${action.type}-${index}`} onClick={() => onChoose(action)}>
              <span>
                ...
                {
                  <em>
                    {previewDeltaText(action.preview_delta, action.cost ? `消耗 ${action.cost} 行动点` : '确认后结算')}
                  </em>
                }
              </span>
              <ChevronDown size={16} />
...
function RoleUpgradeDialog({
  ...
  const options = (state.pending_choice?.options || []) as Array<{
    id: string;
    name?: string;
    description?: string;
    trigger?: string;
  ...
  return (
    <div className="dialog-backdrop">
      ...
      >
        ...
        <h2 id="upgrade-dialog-title">为下一段旅程选择持续专长</h2>
        <p>专长会保留到本局结束，并在描述的触发条件满足时生效。</p>
        <div className="upgrade-options">
          {options.map((option) => {
            const action = choices.find((item) => item.upgrade_id === option.id);
            return (
              <button key={option.id} disabled={disabled || !action} onClick={() => action && onChoose(action)}>
                <span>
                  <b>{option.name || option.label || '角色专长'}</b>
                  <small>
                    {disabled ? '正在同步选择…' : option.description || '选择后会持续改变该角色的行动方式。'}
                  </small>
                  {option.trigger && <i>触发：{option.trigger}</i>}
                </span>
                <em>{option.strategic_direction || '长期协作'}</em>
              </button>
            );
          })}
        </div>
