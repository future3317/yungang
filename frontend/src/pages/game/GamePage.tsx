...
export function GamePage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data, action) => { queryClient.setQueryData([roomId ? 'room-game' : 'game', roomId || sessionId, roomToken], data); setPreview(null); setSelectedOption(null); setActionMode(null); setEventChoiceOpen(false); if (action.type === 'move' && action.target_id) { setFocus(action.target_id); setInspectorOpen(true); } if (data.viewer?.play_mode === 'local' && action.type === 'end_turn' && data.players[data.shared.active_player_id]) setHandoffName(data.players[data.shared.active_player_id].name); setToast(actionFeedback(action, state, data)); window.setTimeout(() => setToast(''), 4200); },
    onError: error => { if (error instanceof ApiError && error.status === 409) { const current = (error.payload as { detail?: { current_state?: GameState } })?.detail?.current_state; if (current) { queryClient.setQueryData([roomId ? 'room-game' : 'game', roomId || sessionId, roomToken], current); setActionMode(null); setSelectedOption(null); setPreview(null); setToast('状态已同步，请重新选择行动。'); return; } } setActionMode(null); setSelectedOption(null); setPreview(null); setToast(error instanceof Error ? error.message : '同步失败，请稍后重试。'); }
  });
...
function ActionTargetDialog({ option, disabled, onChoose, onCancel }: { option: ActionOption; disabled: boolean; onChoose: (target: ActionOption['targets'][number]) => void; onCancel: () => void }) {
  return <div className="dialog-backdrop"><section className="dialog choice-dialog" role="dialog" aria-modal="true" aria-labelledby="action-target-title" aria-describedby="action-target-description"><button className="dialog-close" disabled={disabled} onClick={onCancel} aria-label="取消目标选择"><X /></button><span className="eyebrow">行动目标</span><h2 id="action-target-title">{option.label}</h2><p id="action-target-description">{option.confirmation || option.description || '选择一个合法目标后继续。'}</p><div className="choice-list">{option.targets.map(target => { const delta = Object.entries(target.preview_delta || {}).filter(([, value]) => typeof value === 'number').map(([key, value]) => `${key} ${Number(value) > 0 ? '+' : ''}${value}`).join(' · '); return <button key={target.id} disabled={disabled} onClick={() => onChoose(target)}><span><b>{target.label}</b><small>{delta || `${option.cost?.ap || 0} AP`}</small></span><ChevronDown size={16} /></button>; })}</div><button className="ghost-button" disabled={disabled} onClick={onCancel}>返回浏览</button></section></div>;
}
