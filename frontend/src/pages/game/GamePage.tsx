...
export function GamePage() {
  ...
  const chooseOption = (option: ActionOption) => { if (!canAct || option.enabled === false) return; if (option.type === 'explore') { setSelectedOption(null); setActionMode('explore'); setInspectorOpen(true); return; } if (option.targets.length) { setSelectedOption(option); if (['move', 'restore', 'survey_route', 'restore_route', 'establish_connection'].includes(option.type)) { setActionMode(option.type as ActionMode); setFocus(active.location); } else setActionMode(null); return; } const action = optionAction(option); if (option.type === 'end_turn') { const weathering = state.shared.weathering_track ?? state.shared.threat ?? 0; const warnings = [active.ap > 0 ? `你还有 ${active.ap} 点行动力未使用` : '', state.shared.current_event_id ? '当前事件将在回合结束时结算' : '', weathering >= Math.max(0, (state.shared.weathering_limit || 5) - 1) ? '风化压力已经接近上限' : ''].filter(Boolean); setPreview({ ...action, description: warnings.length ? `${warnings.join('；')}。确认结束后将进入事件结算。` : '确认结束本角色行动并进入事件结算。' }); return; } if (['use_skill', 'use_node_ability', 'use_upgrade', 'end_planning', 'prepare'].includes(option.type)) setPreview(action); else run(action); };
  ...
  const selectInterpretation = (cardId: string, relation: 'support' | 'conflict' | 'pending') => { const action = legal.find(item => item.type === 'interpret_evidence' && item.card_id === cardId && item.target_id === relation); if (action) selectAction(action); };
...
function MobileHandPanel({ id, hidden, active, cards, actionOptions, onCard, onChooseOption, onStrategy }: { id: string; hidden: boolean; active: GameState['players'][string]; cards: Record<string, ContentCard>; actionOptions: ActionOption[]; onCard: (id: string) => void; onChooseOption: (option: ActionOption) => void; onStrategy: (option: ActionOption) => void }) {
  return <section id={id} className={`mobile-hand-panel ${hidden ? 'mobile-hidden' : ''}`} aria-label="手牌与角色能力"><div className="section-label"><Archive size={15} />文化牌 <b>{active.hand.length} / 3</b></div><div className="mobile-hand-grid">{active.hand.map(id => <button key={id} className="hand-card" onClick={() => onCard(id)}><img src={assetUrl(cards[id]?.icon_asset)} alt="" /><b>{cards[id]?.name || id}</b><small>{cards[id]?.strategic_role || '打开查看这件文化证据'}</small></button>)}</div>{active.action_hand?.length ? <><div className="section-label"><Sparkles size={15} />策略牌</div><div className="mobile-hand-grid">{active.action_hand.map(id => { const option = actionOptions.find(item => item.type === 'use_action_card' && item.id.endsWith(`:${id}`)); return <button key={id} className="hand-card strategy-card" disabled={!option || option.enabled === false} onClick={() => option && onStrategy(option)}><img src={assetUrl('icon_card_scroll.png')} alt="" /><b>{option?.label || '策略牌'}</b><small>{option?.description || '查看策略牌效果'}</small></button>; })}</div></> : null}<div className="hand-help">点击文化牌查看详情，策略牌先选择其目标再确认结算。</div></section>;
}
