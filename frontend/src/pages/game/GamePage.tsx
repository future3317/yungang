...
export function GamePage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data, action) => { queryClient.setQueryData(['game', sessionId], data); setPreview(null); setActionMode(null); if (action.type === 'move' && action.target_id) { setFocus(action.target_id); setInspectorOpen(true); } setToast(actionFeedback(action)); window.setTimeout(() => setToast(''), 4200); },
    onError: error => { if (error instanceof ApiError && error.status === 409) { const current = (error.payload as { detail?: { current_state?: GameState } })?.detail?.current_state; if (current) { queryClient.setQueryData(['game', sessionId], current); setActionMode(null); setPreview(null); setToast('状态已同步，请重新选择行动。'); return; } } setActionMode(null); setPreview(null); setToast(error instanceof Error ? error.message : '同步失败，请稍后重试。'); }
  });
