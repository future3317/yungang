...
export function GamePage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data, action) => { queryClient.setQueryData([roomId ? 'room-game' : 'game', roomId || sessionId, roomToken], data); setPreview(null); setActionMode(null); if (action.type === 'move' && action.target_id) { setFocus(action.target_id); setInspectorOpen(true); } if (roomId && action.type === 'end_turn' && data.players[data.shared.active_player_id]) setHandoffName(data.players[data.shared.active_player_id].name); setToast(actionFeedback(action)); window.setTimeout(() => setToast(''), 4200); },
