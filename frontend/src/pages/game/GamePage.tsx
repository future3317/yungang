...
function CardDialog({ id, item, action, interpretActions = [], onClose, onUse }: { id: string; item?: ContentCard; action?: Action; interpretActions?: Action[]; onClose: () => void; onUse: (action?: Action) => void }) { const ref = useDialogFocus(); const relationLabels: Record<string, string> = { support: '支持', conflict: '冲突', pending: '待确认' }; return <div className="dialog-backdrop"><section ref={ref} className="dialog card-dialog" role="dialog" aria-modal="true" aria-labelledby="card-dialog-title"><button className="dialog-close" onClick={onClose} aria-label="关闭"><X /></button><img className="dialog-card-art" src={assetUrl(item?.icon_asset)} alt="" /><span className="eyebrow">文化证据</span><h2 id="card-dialog-title">{item?.name || id}</h2><p>{item?.description || item?.summary || '一份等待被理解并投入合适地点的文化记录。'}</p><div className="card-how-to-use"><b>两种用法</b><span>作为证据会进入研究台并消耗 1 AP；发动即时效果会弃置这张牌，立即改变资源或现场。请选择你现在需要的用途。</span></div>{item?.combo_name && <p className="card-combo"><b>{item.combo_name}</b> · {item.combo_reward_text || item.instant_use_text}</p>}{interpretActions.length > 0 && <div className="card-action-group"><b>作为证据研判</b>{interpretActions.map(candidate => <button key={candidate.target_id} className="secondary-action" onClick={() => onUse(candidate)}>归入{relationLabels[candidate.target_id || ''] || '研究台'} · 消耗 {candidate.cost || 1} AP</button>)}</div>}<div className="card-action-group"><b>发动即时效果并弃置</b><button className="primary-cta" disabled={!action} onClick={() => onUse(action)}>{action ? '查看即时效果' : '当前不可使用'}</button></div></section></div>; }
