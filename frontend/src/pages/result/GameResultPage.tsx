...
export function GameResultPage() {
  ...
  const failureDetail = state.shared.outcome === 'victory' ? '' : state.shared.outcome_reason === 'too_many_closed_sites'
    ...
    : state.shared.outcome_reason === 'weathering_track_reached_limit'
      ...
      : state.shared.outcome_reason === 'round_limit_reached'
        ? `本局已到第 ${state.shared.turn} / ${state.shared.max_rounds} 回合，剩余行动不足以完成共同目标。`
        : '';
  ...
  const objectives = Object.fromEntries((metaQuery.data.objectives || []).map(objective => [objective.id, objective]));
  ...
  return <main className="result-screen"><section className={`result-card ${state.shared.outcome}`}><span className="eyebrow"><Trophy size={16} />{roomId ? '房间旅程结算' : '旅程结算'}</span><h1>{result.title}</h1><p>{state.result?.outcome_summary || result.body}</p>{failureDetail && <div className="result-failure-reason" role="status"><b>这局为什么结束</b><span>{failureDetail}</span></div>}{roomId ? <div className="result-context"><b>{state.viewer?.play_mode === 'multi_device' ? '多设备同行' : state.viewer?.play_mode === 'local' ? '本地协作' : '单人协作'}</b><span>{seatSummary}</span></div> : null}<div className="result-score"><strong>{score.total}</strong><span>团队评分 · {score.grade === 'gold' ? '金' : score.grade === 'silver' ? '银' : '铜'}级</span></div><div className="result-metrics"><span>任务 <b>{score.tasks}</b></span><span>路线 <b>{score.routes}</b></span><span>来源 <b>{score.diversity}</b></span><span>守护 <b>{score.protection}</b></span><span>发现 <b>{score.discovery}</b></span></div>{state.result?.completed_projects?.length ? <div className="result-projects"><span className="eyebrow">已完成项目</span><p>{state.result.completed_projects.map(id => projects[id]?.name || '已完成项目').join(' · ')}</p></div> : null}{state.result?.completed_objectives?.length ? <div className="result-projects"><span className="eyebrow">共同目标</span><p>{state.result.completed_objectives.map(id => objectives[id]?.name || '已完成目标').join(' · ')}</p></div> : null}{eventDetails.length ? <div className="result-projects result-event-log"><span className="eyebrow">事件复盘</span>{eventDetails.map((item, index) => <div className="result-event-entry" key={`${item.name}-${index}`}><b>{item.name}</b><span>影响：{item.targets.length ? item.targets.join('、') : '本轮未记录具体地点'}</span>{item.reason && <small>{item.reason}</small>}<StateChangeList compact changes={item.changes} /></div>)}</div> : null}<div className="result-explanation"><span className="eyebrow">评分依据</span><p>任务与项目体现解释完成度；路线与守护体现网络稳定度；来源与发现体现研究广度；效率分取决于剩余回合。</p></div><p className="result-seed">复盘种子：{state.seed ?? '本局随机'}</p><div className="result-actions">{roomId ? <button className="primary-cta" onClick={() => navigate(`/room/${roomId}`)}><RotateCcw size={17} />返回房间</button> : <><button className="primary-cta" disabled={replay.isPending} onClick={() => replay.mutate(true)}><RotateCcw size={17} />同一种子复盘</button><button onClick={() => replay.mutate(false)}><Compass size={17} />开启新旅程</button></>}<button onClick={() => navigate('/')}><Archive size={17} />返回首页</button></div></section></main>;
}
