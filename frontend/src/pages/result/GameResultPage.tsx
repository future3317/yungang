...
export function GameResultPage() {
  ...
  const gameQuery = useQuery<GameState>({ queryKey: ['game', sessionId], queryFn: () => api.game(sessionId) });
  ...
  if (gameQuery.isError || !gameQuery.data) return <main className="result-screen"><h1>无法读取旅程结算</h1><button onClick={() => navigate('/')}>返回首页</button></main>;
  const state = gameQuery.data;
  ...
  return <main className="result-screen"><section className={`result-card ${state.shared.outcome}`}><span className="eyebrow"><Trophy size={16} />旅程结算</span><h1>{result.title}</h1><p>{result.body}</p><div className="result-score"><strong>{score.total}</strong><span>团队评分 · {score.grade === 'gold' ? '金' : score.grade === 'silver' ? '银' : '铜'}级</span></div><div className="result-metrics"><span>任务 <b>{score.tasks}</b></span><span>路线 <b>{score.routes}</b></span><span>来源 <b>{score.diversity}</b></span><span>守护 <b>{score.protection}</b></span><span>发现 <b>{score.discovery}</b></span></div><p className="result-seed">复盘种子：{state.seed ?? '本局随机'}</p><div className="result-actions"><button className="primary-cta" disabled={replay.isPending} onClick={() => replay.mutate(true)}><RotateCcw size={17} />同一种子复盘</button><button onClick={() => replay.mutate(false)}><Compass size={17} />开启新旅程</button><button onClick={() => navigate('/')}><Archive size={17} />返回首页</button></div></section></main>;
}
