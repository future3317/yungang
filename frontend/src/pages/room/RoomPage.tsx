...
export function RoomPage() {
  ...
  if (roomQuery.isError || !room || !metaQuery.data) { const missing = roomQuery.error instanceof ApiError && roomQuery.error.status === 404; const metadataError = metaQuery.isError; return <main className="room-screen"><section className="room-card"><h1>{missing ? '找不到这段旅程' : metadataError ? '旅程资料暂时无法读取' : '旅舍暂时无法读取'}</h1><p>{missing ? '房间码可能已失效或输入有误。请回到首页创建新的旅程。' : metadataError ? '场景和角色资料暂时没有回应，房间本身没有被删除。请重新读取。' : '可能是网络暂时中断；房间进度仍保存在服务端，可以重新连接。'}</p><div className="room-actions"><button className="primary-cta" disabled={roomQuery.isFetching || metaQuery.isFetching} onClick={() => { void roomQuery.refetch(); void metaQuery.refetch(); }}>{roomQuery.isFetching || metaQuery.isFetching ? '重新连接中…' : '重新读取'}</button><button className="ghost-button" onClick={() => navigate('/')}>返回首页</button></div></section></main>; }
...
function RoleGallery({ roles, seats, activeSeatId, selectedRoleId, takenRoles, managed, onSeatChange, onSelect }: { roles: Meta['roles']; seats: RoomSeat[]; activeSeatId?: string; selectedRoleId: string; takenRoles: Set<string | null | undefined>; managed: boolean; onSeatChange: (seatId: string) => void; onSelect: (roleId: string) => void }) {
  ...
  const otherTeamRoles = new Set(seats.filter(seat => seat.seat_id !== activeSeatId).map(seat => roles.find(role => role.id === seat.role_id)?.team_role).filter(Boolean));
  const roleFit = selected?.team_role && !otherTeamRoles.has(selected.team_role)
  ...
  return <section className="role-selection-panel" aria-labelledby="role-selection-title">
    ...
    <div className="role-selection-layout"><div className="role-card-grid">{roles.map(role => { const asset = roleAsset(role); const unavailable = takenRoles.has(role.id) && role.id !== selectedRoleId; return <button key={role.id} className={`role-choice-card ${role.id === selectedRoleId ? 'selected' : ''} ${unavailable ? 'unavailable' : ''} role-accent-${asset.accent}`} disabled={unavailable} onClick={() => onSelect(role.id)} aria-pressed={role.id === selectedRoleId}><span className="role-choice-portrait"><img src={assetUrl(`ornaments/${asset.portrait}`)} alt="" /><img className="role-choice-badge" src={assetUrl(asset.badge)} alt="" /></span><span className="role-choice-copy"><b>{role.name}</b><small>{role.origin || '同行角色'} · {role.team_role || '协作专长'}</small><em>{role.ability?.name || '角色专长'} · {role.ability?.ap_cost || 0} AP</em></span><ChevronRight className="role-choice-arrow" size={17} /></button>; })}</div><aside className={`role-detail-card role-accent-${selectedAsset.accent}`}><div className="role-detail-art"><img src={assetUrl(`ornaments/${selectedAsset.portrait}`)} alt="" /><img className="role-detail-badge" src={assetUrl(selectedAsset.badge)} alt="" /></div><div className="role-detail-heading"><span>{selected?.origin || '同行角色'}</span><h3>{selected?.name || '选择一位角色'}</h3><p>{selected?.team_role || '为团队补上关键的一束光'}</p></div><div className="role-fit-note"><b>队伍适配</b><span>{roleFit}</span></div><div className="role-detail-section"><span><Sparkles size={14} />专长</span><b>{selected?.ability?.name || '等待选择'}</b><p>{selected?.ability?.description || '选择角色后，这里会显示专长的实际效果。'}</p></div><div className="role-detail-section"><span><MapPin size={14} />行动风格</span><p>{selected?.play_style || selected?.meaning || '在地图上寻找最适合自己的协作位置。'}</p></div>{selected?.solo_rule && <div className="role-solo-note"><b>单人提示</b><span>{selected.solo_rule}</span></div>}{selected?.starting_hint && <div className="role-start-note">{selected.starting_hint}</div>}</aside></div>
  </section>;
}
