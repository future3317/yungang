...
export function RoomPage() {
  ...
  const roomQuery = useQuery<Room>({
    ...
    queryFn: () => api.room(roomId, token || undefined),
  ...
  useRoomEvents({
    ...
    token,
    onRevision: () => {
      void queryClient.invalidateQueries({ queryKey: ['room', roomId, token] });
    },
  ...
  useEffect(() => {
    ...
    else if ((room?.status === 'in_progress' || room?.status === 'paused') && token && viewer?.seat_id)
      navigate(`/room/${roomId}/game`, { replace: true });
  }, [navigate, room?.status, roomId, token, viewer?.seat_id]);

  const showError = (error: unknown) =>
    setFeedback(error instanceof ApiError ? error.message : '网络暂时没有回应，请重试。');
  const update = (promise: Promise<Room>) =>
    promise.then((next) => {
      setFeedback('');
      queryClient.setQueryData(['room', roomId, token], next);
      return next;
    });
  const join = useMutation({
    mutationFn: () => api.joinRoom(roomId, name, roleId || undefined),
    onSuccess: (result) => {
      setFeedback('');
  ...
  const start = useMutation({
    mutationFn: () => api.roomStart(roomId, token),
    onSuccess: () => navigate(`/room/${roomId}/game`, { replace: true }),
    onError: showError,
  });
  const ready = useMutation({
    mutationFn: (next: boolean) => update(api.roomReady(roomId, token, next)),
    onError: showError,
  });
  ...
  if (roomQuery.isLoading || metaQuery.isLoading)
    return (
  ...
  if (roomQuery.isError || !room || !metaQuery.data) {
    const missing = roomQuery.error instanceof ApiError && roomQuery.error.status === 404;
    const metadataError = metaQuery.isError;
    return (
      <main className="room-screen">
        <section className="room-card">
          <h1>{missing ? '找不到这段旅程' : metadataError ? '旅程资料暂时无法读取' : '旅舍暂时无法读取'}</h1>
          <p>
            {missing
              ? '房间码可能已失效或输入有误。请回到首页创建新的旅程。'
              : metadataError
                ? '场景和角色资料暂时没有回应，房间本身没有被删除。请重新读取。'
                : '可能是网络暂时中断；房间进度仍保存在服务端，可以重新连接。'}
          </p>
          <div className="room-actions">
            <Button
              context="room-actions"
              disabled={roomQuery.isFetching || metaQuery.isFetching}
              onClick={() => {
                void roomQuery.refetch();
                void metaQuery.refetch();
              }}
            >
              {roomQuery.isFetching || metaQuery.isFetching ? '重新连接中…' : '重新读取'}
            </Button>
  ...
  const allConfigured =
    room.seats.length === room.max_players && room.seats.every((item) => Boolean(item.role_id) && item.ready);
  const canStart = isHost && allConfigured;
  const selectRole = (nextRoleId: string) => {
    if (room.play_mode === 'multi_device' && !token) {
  ...
  const title =
    room.play_mode === 'solo' ? '配置两位同行角色' : room.play_mode === 'local' ? '配置本地同行席位' : '等待同行者入席';
  const description =
    room.play_mode === 'solo'
      ? '你将轮流调度两位角色。为两席选择不同角色并准备后，即可开始。'
      : room.play_mode === 'local'
        ? '在同一设备完成全部席位配置；行动回合会在席位之间交接。'
        : '每位同行者在自己的设备上选择角色并准备；全部就绪后由房主开始。';
  const scenarioId = (room as Room & { scenario_id?: string }).scenario_id;
  const difficultyId = (room as Room & { difficulty_id?: string }).difficulty_id;
  const scenario = (metaQuery.data.scenarios || []).find((item) => item.id === scenarioId);
      ...
      <section className="room-card" aria-labelledby="room-title">
        <div className="room-card-heading">
          <div>
            ...
            <p>{description}</p>
            <div className="team-role-summary" aria-label="队伍定位">
              <b>队伍分工</b>
              <small>
                {missingRoles.length
                  ? `当前队伍缺少：${missingRoles.map((role) => role.team_role || role.name).join('、')}`
                  : '当前队伍已覆盖全部四类分工。'}
              </small>
              {roles.map((role) => (
                <span key={role.id} className={selectedRoleIds.has(role.id) ? 'covered' : 'missing'}>
                  <i>{selectedRoleIds.has(role.id) ? '已配置' : '可补强'}</i>
                  {role.team_role || role.name}
                </span>
              ))}
              <small>角色专长会改变行动效率；选人时优先补上队伍当前最缺的分工。</small>
            </div>
          </div>
          <div className="room-code">
            <small>{isManagedMode ? '旅程编号' : '房间码'}</small>
            <b>{room.room_id}</b>
            <button
              onClick={() => void navigator.clipboard?.writeText(room.room_id)}
              aria-label={isManagedMode ? '复制旅程编号' : '复制房间码'}
            >
              <Copy size={15} />
            </button>
          </div>
        ...
        <div className="room-seats">
          {Array.from({ length: room.max_players }, (_, index) => {
            const item = room.seats[index];
            return (
              <SeatCard
                key={item?.seat_id || index}
                seat={item}
                index={index}
                roles={roles}
                takenRoles={takenRoles}
                editable={Boolean(item && isHost && (isManagedMode || item.seat_id === 'seat-1'))}
                onUpdate={(next) => item && seat.mutate({ seatId: item.seat_id, update: next })}
              />
            );
          })}
        </div>
        {canPickRole && (
          <RoleGallery
            roles={roles}
            seats={room.seats}
            activeSeatId={activeSeat?.seat_id}
        ...
        {startedWithoutToken ? (
              ...
              <select value={selectedReconnectSeatId} onChange={(event) => setReconnectSeatId(event.target.value)}>
                {room.seats.map((seat) => (
                  <option key={seat.seat_id} value={seat.seat_id}>
                    {seat.name} · {roles.find((role) => role.id === seat.role_id)?.name || '尚未选择角色'}
                  </option>
                ))}
              </select>
            </label>
        ...
        ) : room.play_mode === 'multi_device' && !token && room.status === 'lobby' ? (
          <section className="join-panel">
            <div className="tab-kicker">
              <Users size={15} />
              加入这段旅程
            ...
            <label className="role-join-select">
              选择角色
              <select value={roleId} onChange={(event) => setRoleId(event.target.value)}>
                <option value="">选择角色</option>
                {roles.map((item) => (
                  <option key={item.id} value={item.id} disabled={takenRoles.has(item.id)}>
                    {item.name}
                  </option>
                ))}
              </select>
            </label>
            <p className="role-join-note">先在上方选择一位角色，再加入空闲席位。</p>
            <Button
              context="room-card"
              disabled={join.isPending || room.seats.length >= room.max_players || !roleId}
              onClick={() => join.mutate()}
            >
              <DoorOpen size={16} />
              加入席位
          ...
          <section className="seat-control">
            <div className="tab-kicker">
              <Shield size={15} />
              你的席位
            </div>
            <div className="seat-control-row">
              <div>
                <b>{viewer?.name || '同行者'}</b>
                <small>{roles.find((item) => item.id === viewer?.role_id)?.name || '尚未选择角色'}</small>
              </div>
              <div className="seat-buttons">
                <label className="seat-role-select">
                  &#x9009;&#x62E9;&#x89D2;&#x8272;
                  <select
                    value={viewer?.role_id || ''}
                  ...
                  >
                    ...
                    {roles.map((item) => (
                      <option
                        key={item.id}
                        value={item.id}
                        disabled={takenRoles.has(item.id) && item.id !== viewer?.role_id}
                      >
                        {item.name}
                      </option>
                    ))}
                ...
                <button
                  className={viewer?.ready ? 'ready-button' : 'primary-action'}
                  disabled={!viewer?.role_id}
                  onClick={() => ready.mutate(!viewer?.ready)}
                >
                  {viewer?.ready ? (
                    <>
                      <Check size={15} />
...
function RoleGallery({
  ...
  const otherTeamRoles = new Set(
    seats
      .filter((seat) => seat.seat_id !== activeSeatId)
      .map((seat) => roles.find((role) => role.id === seat.role_id)?.team_role)
      .filter(Boolean)
  );
  const roleFit =
    selected?.team_role && !otherTeamRoles.has(selected.team_role)
    ...
    <section className="role-selection-panel" aria-labelledby="role-selection-title" aria-busy={selectionPending}>
      <div className="role-selection-heading">
        <div>
          ...
          <h2 id="role-selection-title">选择你的同行者</h2>
          <p>每位角色都有不同的行动节奏。查看专长和起手建议，再决定谁守护哪一张证据卡。</p>
        </div>
        ...
        <div className="role-seat-switcher" role="tablist" aria-label="选择要配置的席位">
          {seats.map((seat, index) => (
            ...
            >
              ...
              <b>{seat.name}</b>
              <small>{roles.find((role) => role.id === seat.role_id)?.name || '等待角色'}</small>
            </button>
          ))}
        </div>
      )}
      <div className="role-selection-layout">
        <div className="role-card-grid">
          {roles.map((role) => {
            ...
            const unavailable = takenRoles.has(role.id) && role.id !== selectedRoleId;
            return (
              <button
                key={role.id}
              ...
              >
                <span className="role-choice-portrait">
                  <img src={assetUrl(`ornaments/${asset.portrait}`)} alt="" />
                  <img className="role-choice-badge" src={assetUrl(asset.badge)} alt="" />
                </span>
                <span className="role-choice-copy">
                  <b>{role.name}</b>
                  <small>
                    {role.origin || '同行角色'} · {role.team_role || '协作专长'}
                  </small>
                  <em>
                    {role.ability?.name || '角色专长'} · {role.ability?.ap_cost || 0} 行动点
                  </em>
                </span>
                <ChevronRight className="role-choice-arrow" size={17} />
              </button>
            );
          })}
        </div>
        <aside className={`role-detail-card role-accent-${selectedAsset.accent}`}>
          <div className="role-detail-art">
            <img src={assetUrl(`ornaments/${selectedAsset.portrait}`)} alt="" />
            <img className="role-detail-badge" src={assetUrl(selectedAsset.badge)} alt="" />
          </div>
          <div className="role-detail-heading">
            <span>{selected?.origin || '同行角色'}</span>
            <h3>{selected?.name || '选择一位角色'}</h3>
            <p>{selected?.team_role || '为团队补上关键的一束光'}</p>
          </div>
          <div className="role-fit-note">
            <b>队伍适配</b>
            <span>{roleFit}</span>
          </div>
          <div className="role-detail-section">
            ...
            <b>{selected?.ability?.name || '等待选择'}</b>
            <p>{selected?.ability?.description || '选择角色后，这里会显示专长的实际效果。'}</p>
          </div>
          <div className="role-detail-section">
            <span>
              <MapPin size={14} />
            ...
            <p>{selected?.play_style || selected?.meaning || '在地图上寻找最适合自己的协作位置。'}</p>
          </div>
          {selected?.solo_rule && (
            <div className="role-solo-note">
              <b>单人提示</b>
              <span>{selected.solo_rule}</span>
            </div>
          )}
          {selected?.starting_hint && <div className="role-start-note">{selected.starting_hint}</div>}
        </aside>
      </div>
    </section>
  );
...
function SeatCard({
  ...
  takenRoles: Set<string | null | undefined>;
  editable: boolean;
  onUpdate: (update: { name?: string; role_id?: string; ready?: boolean }) => void;
}) {
  const selected = roles.find((item) => item.id === seat?.role_id);
  if (!seat)
    return (
      <article className="room-seat empty">
        <div className="seat-number">{index + 1}</div>
        <div className="seat-person">
          <b>等待同行者</b>
          <small>房间码会留出这一席位</small>
        </div>
      </article>
    );
  return (
    <article className={`room-seat occupied ${seat.ready ? 'ready' : ''}`}>
      <div className="seat-number">{index + 1}</div>
      <div className="seat-person">
        {editable ? (
          <input
        ...
        <small>{selected?.name || '尚未选择角色'}</small>
      </div>
      {editable ? (
        <div className="seat-buttons">
          <select
            value={seat.role_id || ''}
            onChange={(event) => onUpdate({ role_id: event.target.value })}
            aria-label={`席位 ${index + 1} 角色`}
          >
            <option value="">选择角色</option>
            {roles.map((item) => (
              <option key={item.id} value={item.id} disabled={takenRoles.has(item.id) && item.id !== seat.role_id}>
                {item.name}
              </option>
            ))}
          </select>
          <button
            className={seat.ready ? 'ready-button' : 'primary-action'}
            disabled={!seat.role_id}
            onClick={() => onUpdate({ ready: !seat.ready })}
          >
            {seat.ready ? (
              <>
                <Check size={13} />
