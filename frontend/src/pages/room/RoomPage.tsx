...
export function RoomPage() {
  ...
  const join = useMutation({ mutationFn: () => api.joinRoom(roomId, name, roleId || undefined), onSuccess: result => { setFeedback(''); window.sessionStorage.setItem(tokenKey(roomId), result.seat_token); setToken(result.seat_token); queryClient.setQueryData(['room', roomId, result.seat_token], result.room); }, onError: showError });
  ...
  const leave = useMutation({ mutationFn: () => update(api.roomLeave(roomId, token)), onSuccess: () => { window.sessionStorage.removeItem(tokenKey(roomId)); navigate('/'); }, onError: showError });
