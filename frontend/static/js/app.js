...
const scenes = { yungang: 'scene_yungang_day.png', huayan_temple: 'scene_craft_restoration.png', shanhua_temple: 'scene_archive_cave.png', wall_pass: 'scene_frontier_pass.png', trade_post: 'scene_trade_meeting.png', pingcheng_ruins: 'scene_silk_road_expedition.png' };
...
function renderAction(action, index) {
  ...
  return `<button class="action action-${kind}" data-action-index="${index}"><span class="action-glyph"><img src="/static/ui-assets/generated/${icon}" alt=""></span><span><b>${esc(action.label)}</b><small>${detail}</small></span><i aria-hidden="true">›</i></button>`;
}

function render() {
  const active = state.players[state.shared.active_player_id];
  const focusedSite = state.sites[selectedSite] || state.sites[active.location];
  const focusedScene = scenes[focusedSite.id] || scenes.yungang;
...
function openCard(card, active) {
  ...
  document.querySelector('#modal-root').innerHTML = `<div class="modal-backdrop"><section class="card-modal"><button class="modal-close" aria-label="关闭">×</button><span class="kicker">CULTURE CARD</span><div class="big-card-mark">✦</div><h2>${esc(cards[card] || card)}</h2><p>${text}</p><button class="modal-use" data-use-card="${esc(card)}">${U.use}</button></section></div>`;
...
async function runAction(index, button, active) {
  ...
  try { state = await sendAction({ ...action, action: action.type, player_id: active.id, expected_revision: state.revision }); selectedSite = state.players[state.shared.active_player_id]?.location || null; render(); toast(`${action.label} 路 \u5df2\u5b8c\u6210`, 'success'); }
...
getGame().then(value => { state = value; render(); }).catch(error => { app.innerHTML = `<div class="fatal">\u65e0\u6cd5\u8f7d\u5165\u65c5\u7a0b锛?{esc(error.message)}</div>`; });
