...
- Landing、角色选择页和游戏 HUD 已纳入 `2048 / 1920 / 1440 / 1280` 桌面视觉回归矩阵。
...
当前产品只维护 PC 端。窗口宽度低于 `1024px` 时，应用会显示“请使用桌面端访问”，不会渲染半截游戏界面；因此不再维护移动端布局、移动端测试或移动端视觉基线。桌面视觉回归覆盖首页、角色选择页和 PC 游戏 HUD，固定分辨率项目会在本地服务启动后生成并比对截图。若视觉调整是有意变更，使用 `--update-snapshots` 更新对应基线后必须再次执行一次不带该参数的回归。
...
地图是全屏世界层，左侧角色与行动 HUD、右侧地点检查器叠加在地图之上，桌面端固定为顶部栏、左右信息栏、中间地图和底部行动坞的几何契约。右侧检查器分为任务、项目、事件、市场四页；左右信息栏独立滚动，不改变地图尺寸。地图路线支持 `waypoints`、`roadClass`、`terrain` 和 `labelPosition`，运行时端点始终由路线两端节点计算。
...
当前后端测试、前端类型检查、Vitest、production build、Playwright、axe 和四种桌面分辨率视觉回归均有本地验收入口，必须在本地服务启动后实际执行，不能仅凭代码推断通过。测试使用隔离的临时 SQLite；手动本地运行可能产生 `data/games.sqlite3`、截图、Playwright 报告和 `audit_output/`，这些运行产物不应提交。
